...
with tab5:
    ...
    with an_tab2:
        ...
        if not st.session_state.paper_trade_history and not st.session_state.paper_portfolio:
            ...
            if st.session_state.paper_trade_history:
                ...
                # Trade History Table (Safely display available columns)
                st.markdown("#### 📜 Trade Journal")
    ...
    with an_tab4:
        ...
        r1, r2 = st.columns(2)
        with r1:
            rm_cap = st.number_input("Capital (₹)", 1000, 10000000, 100000, 1000, key="v6_rm_cap")
            rm_entry = st.number_input("Entry Price (₹)", 0.01, 100000.0, 1000.0, 0.5, key="v6_rm_entry")
        with r2:
            rm_sl = st.number_input("Stop Loss (₹)", 0.01, 100000.0, 950.0, 0.5, key="v6_rm_sl")
            rm_target = st.number_input("Target (₹)", 0.01, 100000.0, 1100.0, 0.5, key="v6_rm_target")

        rm_risk = st.slider("Risk Per Trade (%)", 0.5, 10.0, 1.0, 0.5, key="v6_rm_risk")

        if st.button("🧮 Calculate Position Size", key="v6_rm_calc"):
            if rm_entry > rm_sl:
                rps = rm_entry - rm_sl
        ...
        with st.expander("⚙️ Bot & Risk Settings"):
            at_max = st.number_input("Max Positions", 1, 10, 5, key="v6_at_max")
            at_cap = st.number_input("Capital Per Trade (₹)", 1000, 50000, 10000, 1000, key="v6_at_cap")
            at_score = st.slider("Min AI Score", 50, 100, 75, key="v6_at_score")
            at_target_pct = st.slider("Target (%)", 2.0, 20.0, 4.0, 0.5, key="v6_at_target_pct")
            at_sl_pct = st.slider("Stop Loss (%)", 1.0, 10.0, 2.5, 0.5, key="v6_at_sl_pct")

        if st.button("🚀 Run Auto Trade Bot v6.0", key="v6_auto_bot_btn"):
